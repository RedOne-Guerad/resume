...
- 🔐 **Security & PCI-DSS Compliance:** Implemented hardened bastion hosts with CLI session recording (tlog/auditd), MFA authentication, and SSH key-only access.
- 🔌 **NetBox Plugin Development:** Built custom Django plugins for automated Proxmox-to-NetBox VM synchronization with REST API integration and systemd automation.
- 🛠️ **Infrastructure as Code:** Design and implement infrastructure solutions using Ansible and Terraform.
- 🖥️ **Kubernetes:** Manage Kubernetes infrastructure on Linux, including ingress configuration, resource optimization, and troubleshooting OOMKill issues.
...
- 📊 **BI & Data Platforms:** Deploy and maintain Apache Superset, Redash, and Apache NiFi for data analytics and ETL pipelines integrated with ClickHouse.
...
- 📋 **CRM Administration:** Deploy and maintain SuiteCRM with Elasticsearch full-text search and Google Calendar integration.
- 🛡️ **Security Compliance:** Implement security best practices, coordinate CVE patching (OpenSSH RegreSSHion), and conduct regular vulnerability assessments.
- 🔍 **Automation:** Develop automation scripts to reduce manual intervention in repetitive tasks.
- ☁️ **Cloud Infrastructure:** Design scalable cloud solutions leveraging AWS services and following PCI-DSS requirements.
...
- Databases (MySQL, MongoDB, ClickHouse, PostgreSQL) 🗄️
- Programming (Python, Bash, Django) 🐍
- Version Control (Git) 🔄
- Logging & Visualization (Graylog, Sentry, ELK) 📊
- Virtualization (Proxmox VE, VMware) 🖥️
- Load Balancing (HAProxy, NGINX, Keepalived) ⚖️
- DCIM/IPAM (NetBox) 📍
- BI & ETL (Apache Superset, NiFi, Redash) 📉
- CRM Systems (SuiteCRM, Elasticsearch) 📋
- Security & Compliance (PCI-DSS, SSH Hardening) 🔐

---

## 🏗️ Key Projects (NDA - Generalized)

### 🏢 Datacenter Infrastructure Deployment
- Deployment of 4 Proxmox VE clusters (Infrastructure, Production, Kubernetes, Test)
- NetBox implementation as DCIM/IPAM single source of truth
- Custom Django plugin development for automated VM synchronization
- HA load balancer deployment with HAProxy/NGINX/Keepalived
- Hardened bastion host with session recording for compliance

### 📈 Monitoring Platform Modernization
- Upgraded Zabbix monitoring infrastructure to version 7.x
- Integrated Prometheus/Grafana for Kubernetes workload monitoring
- Centralized logging with Graylog
- Custom Python modules for specialized monitoring

### 📊 Data Analytics Platform
- Deployed Apache Superset for business intelligence
- Configured Apache NiFi for ETL pipelines
- Managed ClickHouse clusters for real-time analytics
- Integrated data replication with Kafka
